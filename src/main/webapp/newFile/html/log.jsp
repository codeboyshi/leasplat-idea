...
function serchList()
{
	$.post("./select.do",{pageNo:1,pageSize:10},function(data){
		...
		if(data.error.code == 1){
			...
			pageBean = {"total":data.data.pa,"totalRecords":data.data.totalCount};
			 kkpager.generPageHtml({
			        ...
			        click : function(n){
			        	$.post("./select.do",{pageNo:n,pageSize:10},function(data){
			        		console.log(data);
			        		forEach(data);
			            })
			            this.selectPage(n);
...
function forEach(data){
	...
	for(var i = 0;i < data.data.result.length;i++)
	{
		...
		html += '<td nowrap="">' + data.data.result[i].moduleName + '</td>';
		html += '<td nowrap="">' + data.data.result[i].operationName + '</td>';
		html += '<td nowrap="">' + data.data.result[i].user.userName + '</td>';
