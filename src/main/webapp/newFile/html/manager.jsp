			...
			<div id="container">
				<div class="ctx">
					<div class="search">
						...
						<div class="btn-group">
							<div class="search-btn" id="select" style="background:blue;color:#FFF;"><i class="iconfont">&#xe601;</i>搜索</div>
							<div class="reset-btn" id="reset" style="background:gray;color:#FFF;margin-left:20px;"><i class="iconfont">&#xe626;</i>重置</div>
						</div>
					</div>
					<div class="result">
...
<tr>
									<td nowrap="">石强</td>
									<td nowrap="">12093039100</td>
									<td nowrap="">211232434353453453454</td>
									<td>
										<div class="btn" style="background:red;"><i class="iconfont"></i>删除</div>
										<div class="btn" style="background:#E10000;"><i class="iconfont"></i>禁用</div>
									</td>
									</tr>
 -->
...
{
	$.post("../user/list.do",{pageNo:1,pageSize:10,userName:value},function(data){
		...
		if(data.error.code == 1){
			...
			pageBean = {"total":data.data.pa,"totalRecords":data.data.totalCount};
			 kkpager.generPageHtml({
			        ...
			        click : function(n){
			        	$.post("/leasplat/user/list.do",{pageNo:n,pageSize:10,userName:value},function(data){
			        		console.log(data);
			        		forEach(data);
			            })
			            this.selectPage(n);
...
function forEach(data){
	...
	{
		html += '<tr>';
		html += '<td nowrap="">' + data.data.result[i].userName + '</td>';
		if(data.data.result[i].phone == null)
		{
			html +=	'<td nowrap=""></td>';
		}
		else
		{
			html +=	'<td nowrap="">' + data.data.result[i].phone + '</td>';
		}
		
		if(data.data.result[i].userSex == null)
		{
			html +=	'<td nowrap=""></td>';
		}
		else
		{
			if(data.data.result[i].userSex == 0)
			{
				html +=	'<td nowrap="">男</td>';
			}
			if(data.data.result[i].userSex == 1)
			{
				html +=	'<td nowrap="">女</td>';
			}
		}
		html += '<td>';	
		html += '<div class="btn" style="background:#E10000;" onclick = "deleteUser(' + data.data.result[i].userId + ')"><i class="iconfont"></i>禁用</div>';	
		html += '</td>';
		html += '</tr>';
	}
	$("#bodyMsg").html(html);
...
{
	$.post("../user/delete.do",{userId:id},function(data){
		if(data.error.code == 1)
		{
			alert("禁用成功");
