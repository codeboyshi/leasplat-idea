			...
			<div id="container">
				<div class="ctx">
					...
					<div class="result">
						...
						<a href="../goodsclass/add.do"  
						style="display:block;line-height:40px;text-align:center;
						height:40px;width:80px;background:blue;border-radius:5px;
...
{
	$.post("../goodsclass/select.do",{pageNo:1,pageSize:10,name:value},function(data){
		...
		if(data.error.code == 1){
			...
			pageBean = {"total":data.data.pa,"totalRecords":data.data.totalCount};
			 kkpager.generPageHtml({
			        ...
			        click : function(n){
			        	$.post("../goodsclass/select.do",{pageNo:n,pageSize:10,name:value},function(data){
			        		console.log(data);
			        		forEach(data);
			            })
			            this.selectPage(n);
...
function forEach(data){
	...
	{
		html += '<tr>';
		html += '<td nowrap="">' + data.data.result[i].name + '</td>';
		var date = (new Date(data.data.result[i].createTime));
		
		html +=	'<td>' + (1900+date.getYear()) + '年' + (date.getMonth() + 1) + '月' + date.getDate() + '日' + '' + '</td>';
		html += '<td>';	
		html += '<div class="btn" style="background:blue;" onclick = "edit(' + data.data.result[i].id + ')"><i class="iconfont">&#xe61b;</i>修改</div>';
		html += '<div class="btn" style="background:#E10000;" onclick = "deleteG(' + data.data.result[i].id + ')"><i class="iconfont"></i>删除</div>';	
		html += '</td>';
		html += '</tr>';
	}
	$("#bodyMsg").html(html);
...
function deleteG(id)
{
	$.post("../goodsclass/delete.do",{id:id},function(data){
		if(data.error.code == 1)
		{
			alert("删除成功");
...
function edit(id)
{
	window.location.href = "../goodsclass/edit.do?id=" + id;
}
</script>
</html>
