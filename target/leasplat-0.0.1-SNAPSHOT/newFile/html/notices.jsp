...
{
	$.post("../notice/view.do",{pageNo:1,pageSize:5,name:value},function(data){
		...
		if(data.error.code == 1){
			...
			pageBean = {"total":data.data.pa,"totalRecords":data.data.totalCount};
			 kkpager.generPageHtml({
			        ...
			        click : function(n){
			        	$.post("../notice/view.do",{pageNo:n,pageSize:5,name:value},function(data){
			        		console.log(data);
			        		forEach(data);
			            })
			            this.selectPage(n);
...
function forEach(data){
	...
	{
		html += '<tr>';
		html += '<td nowrap="">' + data.data.result[i].noticeMessage + '</td>';
		var date = (new Date(data.data.result[i].createDate));
...
function deleteG(id)
{
	$.post("../notice/delete.do",{id:id},function(data){
		if(data.error.code == 1)
		{
			alert("删除成功");
...
function edit(id)
{
	window.location.href = "../notice/edit.do?id=" + id;
}
</script>
</html>
